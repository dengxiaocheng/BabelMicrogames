...
rm -f "$PREFIX/bin/s"
cat << 'EOF' > "$PREFIX/bin/s"
#!/data/data/com.termux/files/usr/bin/sh
# 这个启动器负责把 Termux 的 s 入口重写成 online 固定线程的远端手动会话入口。
...
fifo=$(mktemp -u "$state_dir/termux_s.XXXXXX.fifo") || exit 1
pidfile=$(mktemp "$state_dir/termux_s.XXXXXX.pid") || exit 1
reasonfile=$(mktemp "$state_dir/termux_s.XXXXXX.reason") || exit 1

# 这些本地临时文件只服务于当前一次 s 会话。
cleanup_local_state() {
...
heartbeat_loop() {
  while :; do
    ...
    ssh -o StrictHostKeyChecking=no root@"$target_host" \
      "su - openclaw -c 'sh $remote_helper --action touch-lease --workdir /home/openclaw/babel-runtime --bridge-root /home/openclaw/babel-runtime --thread-id 019dbcf2-d632-7770-9cbc-ae1f963eb2e0 --entrypoint termux_s --session-id $session_id'" \
      >/dev/null 2>&1 || true
...
shift
printf "%s\n" "$$" > "$pidfile"
exec ssh -o StrictHostKeyChecking=no -tt root@"$1" "su - openclaw -c '\''sh $2 --action manual-resume --workdir /home/openclaw/babel-runtime --bridge-root /home/openclaw/babel-runtime --thread-id 019dbcf2-d632-7770-9cbc-ae1f963eb2e0 --entrypoint termux_s --session-id $3 --lease-ttl-seconds $4 --lease-check-seconds $5'\''"
