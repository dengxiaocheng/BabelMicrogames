...
/**
 * NightPrayerGame — thin controller wrapping the night-prayer state engine.
 * Node entry point; also exported for programmatic use / tests.
 */
export class NightPrayerGame {
  constructor() {
  ...
  /** Return events eligible for the current night. */
  getCurrentEvents() {
...
async function main() {
  const game = new NightPrayerGame();
  ...
  while (!game.isOver()) {
    ...
    console.log(
      `-- ${s.phaseName} | 夜 ${s.night} | ` +
      `精神 ${s.spirit}  怀疑 ${s.suspicion}  信仰 ${s.faith} --\n`,
    );
